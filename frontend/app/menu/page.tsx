...
function getAllReceiptsFromCache(): StoredReceipt[] {
  try {
    ...
    return results.sort((a, b) =>
      new Date(b.receiptData.paidAt).getTime() - new Date(a.receiptData.paidAt).getTime()
    );
  } catch { return []; }
}
...
function ReceiptBanner({ onClick, onViewAll }: { onClick: () => void; onViewAll: () => void }) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="flex items-center gap-2 shrink-0">
        <motion.button whileTap={{ scale: 0.94 }} whileHover={{ scale: 1.04 }} onClick={onViewAll}
          className="px-3 py-2 rounded-full text-[#d4a762] text-xs font-semibold tracking-wide border border-[#c49a45]/25 hover:border-[#c49a45]/50 transition">
          All
        </motion.button>
...
function ReceiptsListModal({
  onClose,
  onSelect,
}: {
  onClose: () => void;
  onSelect: (stored: StoredReceipt) => void;
}) {
  const receipts = getAllReceiptsFromCache();
  const card = {
    background: "#080603",
    ...
    <div className="rounded-2xl flex flex-col overflow-hidden" style={{ ...card, maxHeight: "88vh" }}>
      <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-[#1e1508] shrink-0">
        <div className="flex flex-col gap-0.5">
          <p className="text-[#d4a762] text-xs tracking-[0.22em] uppercase font-mono">Order History</p>
          <h3 className="text-white text-xl font-serif">Your Receipts</h3>
        </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 px-4 py-4 flex flex-col gap-3"
        ...
        {receipts.length === 0 ? (
          <p className="text-gray-600 text-sm text-center py-10">No receipts found.</p>
        ) : receipts.map((stored) => {
          const r = stored.receiptData;
          return (
            <motion.button key={r.orderId} whileTap={{ scale: 0.98 }} whileHover={{ scale: 1.01 }}
              onClick={() => onSelect(stored)}
              className="w-full text-left px-4 py-3.5 rounded-xl border border-[#1e1508] hover:border-[#c49a45]/30 transition"
              style={{ background: "rgba(196,154,69,0.04)" }}>
              <div className="flex items-center justify-between gap-3">
                <div className="flex flex-col gap-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-[#d4a762] text-sm font-mono font-semibold">
                      #{r.orderId.slice(-8).toUpperCase()}
                    </span>
                    <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded-full ${r.paymentStatus === "SUCCESS"
                      ? "text-[#7ec87e] bg-[#7ec87e]/10 border border-[#7ec87e]/20"
                      : "text-yellow-400 bg-yellow-400/10 border border-yellow-400/20"
                      }`}>
                      {r.paymentStatus === "SUCCESS" ? "Paid" : r.paymentStatus}
                    </span>
                  </div>
                  <p className="text-gray-500 text-xs font-mono">{formatTimestamp(r.paidAt)}</p>
                  <p className="text-gray-600 text-xs truncate">
                    {r.items.map(i => `${i.name} ×${i.quantity}`).join(", ")}
                  </p>
                </div>
                <div className="flex flex-col items-end gap-1 shrink-0">
                  <span className="text-[#c49a45] text-base font-serif font-semibold">₹{r.total.toFixed(2)}</span>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#4a3820" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" />
                  </svg>
                </div>
              </div>
      ...
      <div className="shrink-0 px-5 pb-5 pt-3 border-t border-[#1e1508] bg-[#080603]">
        <p className="text-[#3a2a10] text-center text-xs font-mono">
          Showing receipts cached in this browser · expires in 2 hrs
        </p>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {showReceiptBanner && (
          <ReceiptBanner
            ...
            onViewAll={() => setModal({ type: "receipts_list" })}
          />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        ...
        {modal.type === "receipts_list" && (
          <Backdrop key="receipts-list" onClose={() => setModal({ type: "idle" })} wide>
            <ReceiptsListModal
              onClose={() => setModal({ type: "idle" })}
              onSelect={(stored) => setModal({
                type: "receipt",
                imageBase64: stored.imageBase64,
                receiptData: stored.receiptData,
              })}
            />
          </Backdrop>
        )}
